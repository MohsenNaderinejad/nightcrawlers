...
class NMOVV3Server(NetWorkMultiplayerServer):
    def __init__(self, Arena_ID: int, PINFS, P1_CONN, P2_CONN, P3_CONN):
        super().__init__(Arena_ID, MMode.OVV3, PINFS)
        self.rounds_to_win_for_each_team = 2
        self.match_max_timer = (60 * 60 * 2.5) + 1
        ...
        self.str_match_end = f"{self.P1.name}/{self.P1.hashtag} vs {self.P2.name}/{self.P2.hashtag} vs {self.P3.name}/{self.P3.hashtag}"
    ...
    def update(self):
        ...
        if atleast_a_player_is_still_alive:
            ...
            if self.t1_dead and self.t2_dead and self.t3_dead:
                print("Draw round")
                self.rounds_played -= 1
            elif self.t2_dead and self.t3_dead:
                self.teams[1].rounds_won += 1
            elif self.t1_dead and self.t3_dead:
                self.teams[2].rounds_won += 1
            elif self.t1_dead and self.t2_dead:
